angular.module("orsApp.settings-service", []).factory("orsSettingsFactory", [
  "$timeout",
  "orsObjectsFactory",
  "orsUtilsService",
  "orsRequestService",
  ...
  (
    ...
    orsSettingsFactory.setActiveOptions = (options, fireRequest) => {
      ...
      if (fireRequest)
        orsSettingsFactory[currentSettingsObj].onNext(
    ...
    orsSettingsFactory.updateWaypoints = () => {
      ...
      orsSettingsFactory[currentWaypointsObj].onNext(
        orsSettingsFactory[currentSettingsObj].getValue().waypoints
      );
    };
    ...
    orsSettingsFactory.setAvoidableAreas = avoidablePolygons => {
      orsSettingsFactory[
        currentSettingsObj
      ].getValue().avoidable_polygons = avoidablePolygons;
      orsSettingsFactory[currentSettingsObj].onNext(
        orsSettingsFactory[currentSettingsObj].getValue()
      );
    };
    ...
    orsSettingsFactory.routingSettingsSubject.subscribe(settings => {
      console.info(
        "changes in routingSettingsSubject",
      ...
      request.promise.then(
        data => {
          if (data.features.length > 0) {
            const addressData = orsUtilsService.addShortAddresses(
              data.features
            )[0];
            let addressStrings = [
              addressData.processed.primary,
              addressData.processed.secondary
            ];
