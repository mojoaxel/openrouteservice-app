angular.module('orsApp').directive('orsMap', () => {
    return {
        ...
        controller: ['$scope', '$filter', '$compile', '$timeout', '$window', 'orsSettingsFactory',  'orsObjectsFactory', 'orsRequestService', 'orsUtilsService', 'orsMapFactory', 'orsCookiesFactory', 'lists', 'globals', 'mappings', 'orsNamespaces', 'ENV', ($scope, $filter, $compile, $timeout, $window, orsSettingsFactory, orsObjectsFactory, orsRequestService, orsUtilsService, orsMapFactory, orsCookiesFactory, lists, globals, mappings, orsNamespaces, ENV) => {
            ...
            $scope.mapStyleId = (orsCookiesFactory.getMapOptions() && orsCookiesFactory.getMapOptions().msi) ? orsCookiesFactory.getMapOptions().msi : 3;
            ...
            $scope.mapModel.map.on("load", (evt) => {
                ...
                $scope.smallScreen = $scope.getWindowDimensions().w < 720 ? true : false;
            });
            /**
            ...
            $scope.addFeatures = (actionPackage) => {
                const isDistanceMarkers = orsSettingsFactory.getUserOptions().showDistanceMarkers === true ? true : false;
            ...
            */
            ...
            $scope.addHoverPoint = (mapModel, hoverPolyLine, pointList, latlng) => {
                ...
                $scope.hoverPoint = new L.marker(snappedPosition.latlng, {
                    ...
                    draggable: 'true',
                    //bubblingMouseEvents: true
                }).addTo(mapModel.geofeatures.layerRouteDrag).on('dragend', (event) => {
            ...
            $scope.addIsochrones = (actionPackage) => {
                const randomColorsSelected = orsSettingsFactory.getUserOptions().randomIsochronesColors === true ? true : false;
...
angular.module('orsApp').directive('orsRoutePointPopup', ['$translate', ($translate) => {
    return {
        restrict: 'E',
...
angular.module('orsApp').directive('orsSignupBox', ['$translate', ($translate) => {
    return {
        restrict: 'E',
